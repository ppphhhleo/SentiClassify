...
class LSTM_Network(nn.Module):
	def __init__(self, emb_matrix):
		...
		self.lstm = nn.GRU(input_size=300, hidden_size=300, batch_first=True, bidirectional=False) #LSTM层，自行调参，如果bidirectional设置True则为BiLSTM，但是相应的hidden_size记得除2
...
class AELSTM_Network(nn.Module):
	def __init__(self, emb_matrix):
		...
		self.lstm = nn.GRU(input_size=300 * 2, hidden_size=300, batch_first=True, bidirectional=False)
...
class ATAELSTM_Network(nn.Module):
	...
	# attention机制，如global_attention, local_attention, multi-head_attention等
	def attention(self, K, mask=None):
		...
		if mask is not None: # mask矩阵没有生成
			mask = mask.bool()
...
#AT—LSTM模型
class ATLSTM_Network(nn.Module):
	def __init__(self, emb_matrix):
		super(ATLSTM_Network, self).__init__()
		self.embedding = nn.Embedding.from_pretrained(emb_matrix, freeze=False)
		self.lstm = nn.GRU(input_size=300 , hidden_size=300, batch_first=True, bidirectional=False)
